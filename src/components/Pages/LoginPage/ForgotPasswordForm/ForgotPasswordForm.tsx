...
const ForgotPasswordForm: React.FC<Props> = (props: Props) => {
  ...
  const handleChangeForgotPassword: (event: React.ChangeEvent<any>) => void = (
    event
  ) => {
  ...
  const handleSubmitForgotPassword: (event: React.FormEvent) => void = (
    event
  ) => {
    ...
    axios
      .post(
        "https://city-walks.herokuapp.com/forgot-password",
        qs.stringify(payload)
      )
      .then((res: AxiosResponse) => {
        alert(
          "We have sent you an email. Please click the click in your email to reset your password"
        );
        setForgotPasswordEmail("");
        window.scrollTo(0, 0);
      })
