...
const ThreadBox: React.FC<Props> = (props: Props) => {
  ...
  const handleSubmit: (event: React.FormEvent) => void = (event) => {
    ...
    axios
      .post(
        "https://city-walks-production.up.railway.app/add-reply",
      ...
      return (
        <div className='reply' key={index}>
          ...
          {reply.userId === currentUserId && (
            <MDBBtn outline onClick={() => handleDeleteReply(reply._id)}>
              Delete
            </MDBBtn>
          )}
        </div>
  ...
  return (
    <MDBCard className='card-body thread-box'>
      <div className='thread-container'>
        ...
        <MDBCardText className='thread-box-content'>{content}</MDBCardText>
        <p>
          Posted by {userFirstName} on{" "}
          {submittedOn.replace("T", " ").substring(0, 19)}
        </p>
        {userId === currentUserId && (
          <MDBBtn
            outline
      ...
      <MDBBtn
        outline
        color='default'
        className='show-all-replies-btn'
